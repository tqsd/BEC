...
# Your factory (the corrected one that matches GaussianEnvelopeU API)
# from bec.light.classical.factories.pi_pulses import (
#    make_gaussian_field_drive_pi,
#    drive_spec_from_field_drive,
#
# )
...
def main() -> None:
    ...
    # solver grid in "ps units" (since time_unit_s = 1 ps)
    tlist = np.linspace(0.0, 500.0, 5001)

    # Resonant frequency for G<->X1 (physical rad/s)
    ...
    # Step 2: sanity check compiled problem before solving
    _debug_compiled_problem(problem, drive_label_substr="H_drive_")
    # If you want the exact label, it will include drive_id and pair, e.g. "H_drive_H1_pi_G_X1"
    ...
    figs = plot_runs(
        ...
        qds=[qd],
        cfg=PlotConfig(
            title="Single scenario: pi Gaussian H pulse (G->X1)",
            ncols=1,
            column_titles=["pi Gaussian H (G->X1)"],
            show_omega_L=True,
