...
def _coerce_drives(drive: Optional[Any]) -> Sequence[Any]:
    if drive is None:
        return ()
...
def _maybe_eval_delta_omega_rad_s(
    ...
    - drive.carrier.delta_omega is a callable taking physical time
    - drive.carrier.delta_omega.fn is callable
    - drive.carrier has method delta_omega_phys(t)
    """
    ...
    # Case 3: attribute object with .eval or .fn
    if dw is not None:
...
def extract_qd_traces(
    ...
    *,
    units: Any,
    drive: Optional[Any] = None,  # single drive or list/tuple
    ...
    if expect is None:
        raise ValueError("res.expect is required")

    # Populations
    pops = {}
    for k in pop_keys:
        if k in expect:
            arr = _as_1d(expect[k], len(t_s_full), name=k)[sl]
            pops[k] = np.asarray(np.real(arr), dtype=float)
    ...
    for k in out_keys:
        if k in expect:
            arr = _as_1d(expect[k], len(t_s_full), name=k)[sl]
            outputs[k] = np.asarray(np.real(arr), dtype=float)
    ...
    for k, arr in expect.items():
        if isinstance(k, str) and k.startswith(coherence_prefix):
            coherences[k] = np.asarray(
                _as_1d(arr, len(t_s_full), name=k)[sl], dtype=complex
            )
    ...
    if extra_keys is not None:
        for k in extra_keys:
            if k in expect:
                extra[k] = np.asarray(
                    _as_1d(expect[k], len(t_s_full), name=k)[sl]
                )

    # Drives
    drives_in = _coerce_drives(drive)
    ...
    for i, d in enumerate(drives_in):
        ...
        if hasattr(d, "E_env_V_m"):
            try:
                E = np.asarray(
                    [float(d.E_env_V_m(float(ts))) for ts in t_s], dtype=float
                )
            except Exception:
                E = None

        if hasattr(d, "omega_L_rad_s"):
            try:
                tmp = [d.omega_L_rad_s(float(ts)) for ts in t_s]
                if all(x is not None for x in tmp):
                    wL = np.asarray([float(x) for x in tmp], dtype=float)
            except Exception:
                wL = None

        # Best-effort chirp / delta_omega(t)
        try:
            dw = _maybe_eval_delta_omega_rad_s(d, t_s)
        except Exception:
            dw = None

        # Drive-implied Omega(t) = mu * E(t) / hbar
        # Output in rad/s (since (J)/(J*s) = 1/s and we interpret as angular frequency)
