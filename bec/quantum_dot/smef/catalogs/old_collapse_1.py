...
class QDCollapseCatalog(FrozenCatalog):
    ...
    def from_qd(
        ...
        *,
        ...
        # ---- phonon-induced exciton relaxation (phenomenological) ----
        # L = sqrt(gamma) * |dst><src|  == sqrt(gamma) * t_src_dst

        r = _maybe_rate_key(RateKey.PH_RELAX_X1_X2)
        if r is not None:
            g = float(units.rate_to_solver(r))
            if g > 0.0:
                op = _sigma_on_qd(qd_i, Transition.X1_X2)  # |X2><X1|
                terms.append(
                    Term(
                        kind=TermKind.C,
        ...
        r = _maybe_rate_key(RateKey.PH_RELAX_X2_X1)
        if r is not None:
            g = float(units.rate_to_solver(r))
            if g > 0.0:
                op = _sigma_on_qd(qd_i, Transition.X2_X1)  # |X1><X2|
                terms.append(
                    Term(
                        kind=TermKind.C,
                        op=OpExpr.scale(complex(np.sqrt(g)), op),
