...
class QDCollapseCatalog(FrozenCatalog):
    ...
    def from_qd(cls, qd, *, units) -> TermCatalogProto:
        ...
        def rate_value(rk):
            # rates dict might be keyed by strings (rk.value) or by RateKey itself
            if rk in rates:
                r = rates[rk]
            else:
                r = rates.get(getattr(rk, "value", str(rk)))
            if r is None:
                raise KeyError(f"Missing rate for {rk}")
            # r is unitful (1/s). convert to solver using units.
            if units is None:
                raise ValueError(
                    "units must be provided to build collapse catalog"
                )
