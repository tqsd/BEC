...
from bec.quantum_dot.kron_pad_utility import KronPad  # for typing only
from bec.params.transitions import Transition, TransitionType


class FakeModeProvider:
    def __init__(self, modes, intrinsic_len=3):
        # list of objects having .source and .transition
        self.modes = modes[:]
        # CollapseBuilder checks len(self.intrinsic)
        self.intrinsic = [None] * intrinsic_len
...
class CollapseBuilderTests(unittest.TestCase):
    def setUp(self):
        ...
        # Four INTERNAL modes, one for each transition we need
        self.modes = [
            SimpleNamespace(
        ...
        # intrinsic_len != 2 to follow the "else" branch in CollapseBuilder
        self.mode_provider = FakeModeProvider(self.modes, intrinsic_len=3)
        ...
        # Composite dimensions (only product matters to Qobj)
        self.dims = [4, 2, 2]  # total dimension = 16

    @patch("bec.quantum_dot.collapse_builder.interpreter")
    def test_qutip_collapse_ops_calls_interpreter_with_expected_add_tuples(
        ...
        # We get two Qobj collapse operators (two "add" expressions)
        ...
        # Interpreter must be called exactly twice with the two "add" tuples
        ...
        # Assert each call matches one of the expected "add" expressions
        got_ops = [call.args[0] for call in calls]
        self.assertCountEqual(got_ops, expected_ops)

        # And contexts/dims were passed through verbatim
        for call in calls:
            _, got_ctx, got_dims = call.args
