...
from bec.errors.coherence import coherence_HV_VH
from bec.errors.log_negativity import log_negativity_early_late
from bec.errors.two_photon_metrics import two_photon_metrics_multimode
from bec.helpers.pprint import pretty_density, pretty_ket
...
def main():
    ...
    FSS = 1e-6  # fine-structure splitting 10 μeV = 1e-5 eV
    ...
    A_pi = float(np.pi / (omega0 * sigma * np.sqrt(2.0 * np.pi))) * 0.80

    env_json = {
        "type": "symbolic",
        "expr": "A*np.exp(-(t-t0)**2/(2*sigma**2))",
        "params": {"A": A_pi, "t0": t0, "sigma": sigma},
    }
    drive = ClassicalTwoPhotonDrive.from_envelope_json(
        env_json, omega0=2e9, detuning=0.0, label="2g"
    )
