...
def plot_expectations(res, *, time_unit_s: float) -> None:
    ...
    if all(k in expect for k in ["pop_G", "pop_X1", "pop_X2", "pop_XX"]):
        total = _get("pop_G") + _get("pop_X1") + _get("pop_X2") + _get("pop_XX")
        plt.figure()
...
def main() -> None:
    ...
    tlist = np.linspace(0.0, 200.0, 201)  # solver units
    ...
    print("Compiled h_terms:", len(problem.h_terms))
    for i, term in enumerate(problem.h_terms):
        print(i, term.label, term.op.shape)
